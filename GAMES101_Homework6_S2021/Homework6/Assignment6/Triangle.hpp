...
// Convert each triangle from .obj to an object in scene
...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    // Implementation of Moller Trumbore Algorithm based on the geometric insights
    // 1.If cos(theta) > 0, the ray the best case always hit the triangle from the bottom <=> Undefined
    if (dotProduct(ray.direction, normal) > 0)
        return inter;

    // 2. Similar to Moller Trumbore Algorithm: (u + v + w  = 1 && u,v,w > 0 && t > 0)
    double u, v, t_tmp = 0;
    ...
    inter.obj = this;
    inter.m = this->m;
    inter.distance = t_tmp; // distance here stands for the scalar multiple(t)
